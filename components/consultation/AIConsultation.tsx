...
import { Bot, Send, Mic, MicOff, Volume2, VolumeX, StopCircle, AlertCircle, History, BookOpen, TrendingUp, Lightbulb, Clock, Star, Loader2, Ear } from 'lucide-react';
...
import { speechManager, stopAllSpeech as globalStopSpeech } from '../../services/speechManager';
...
export const AIConsultation = forwardRef<any, AIConsultationProps>(({ isFloating = false, autoPrompt = null }, ref) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      content: '您好，我是您的AI健康助手"糖豆"🍬。我注意到您最近的健康数据显示血压略高（118/75 mmHg），且深睡时间较短。\n\n💡 您可以随时说"糖豆"唤醒我进行语音对话，或者直接在下方输入您的问题。',
      timestamp: new Date()
    }
  ]);
  ...
  const [autoVoiceMode, setAutoVoiceMode] = useState(false); // 语音交互模式（唤醒后开启）
  ...
  const [wakeListening, setWakeListening] = useState(false); // 唤醒词监听状态
  ...
  // 组件加载时，检查是否需要自动开始语音识别（被唤醒后打开）
  useEffect(() => {
    // 监听唤醒词服务状态
    const handleWakeListeningChange = (listening: boolean) => {
      setWakeListening(listening);
    };
    wakeWordService.addListener(handleWakeListeningChange);

    // 设置唤醒回调 - 当组件已加载时，直接开始语音识别
    wakeWordService.onWake(() => {
      console.log('🎉 AIConsultation: 收到唤醒信号，开始语音识别');
      setAutoVoiceMode(true);
      // 延迟等待语音播报完成后开始识别
      setTimeout(() => {
        console.log('🎤 开始语音识别...');
        startVoiceRecognitionForWake();
      }, 2000); // 等待"我在，您有什么事吗"播报完成
    });

    // 如果组件刚加载且语音正在播放（说明是被唤醒打开的），等待播报完成后开始识别
    if (speechManager.getIsSpeaking()) {
      console.log('🎤 AIConsultation: 检测到语音播放中，等待完成后开始识别');
      setAutoVoiceMode(true);
    ...
    return () => {
      wakeWordService.removeListener(handleWakeListeningChange);
    };
  }, []);
  ...
  const handleSpeakSummary = () => {
    if (isSpeaking) {
      ...
      speechManager.speak(healthSummary, { rate: 0.9 });
    }
  };
  ...
  const speakText = async (text: string): Promise<void> => {
    ...
    try {
      // 优先尝试使用 Edge-TTS（更自然的语音）
      const result = await textToSpeech(text, 'zh-CN-XiaoyiNeural');
      if (result.success && result.data?.audio_url) {
        await playAudio(result.data.audio_url);
        return;
      }
    } catch (error) {
      console.log('Edge-TTS 不可用，使用浏览器 TTS');
    }
    
    // 备用：使用全局语音管理器
    await speechManager.speak(text, { rate: 0.9 });
  };
  
  // 唤醒后专用的语音识别（跳过状态检查）
  const startVoiceRecognitionForWake = () => {
  ...
  const doStartVoiceRecognition = () => {
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === 'no-speech') {
        ...
        if (autoVoiceMode) {
          setTimeout(() => doStartVoiceRecognition(), 500);
        }
        return;
      }
      ...
      setIsListening(false);
      if (autoVoiceMode) {
        setTimeout(() => doStartVoiceRecognition(), 1000);
      }
    };

    recognition.onend = () => {
      console.log('🔵 语音识别结束, finalTranscript:', finalTranscript);
      if (silenceTimer) clearTimeout(silenceTimer);
      setIsListening(false);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // 播报欢迎语
      speechManager.speak('您好，我是您的AI健康助手糖豆。您可以随时说"糖豆"唤醒我', { rate: 0.9 });
    }, 1500);
  ...
  return (
    <div className="h-full flex flex-col gap-4 p-4 overflow-y-auto">
      ...
      {/* 唤醒词监听状态提示 */}
      {wakeListening && !autoVoiceMode && (
        <Card className="border-2 border-amber-400 bg-amber-50">
          <div className="p-4 flex items-center justify-center gap-4">
            <Ear className="h-6 w-6 text-amber-600 animate-pulse" />
        ...
        <div className="p-4 border-b bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 ml-auto">
              {/* 唤醒词监听按钮 */}
              <Button
                variant={wakeListening ? "default" : "outline"}
                size="sm"
                onClick={toggleWakeMode}
                className={wakeListening ? 'bg-amber-500 hover:bg-amber-600' : ''}
              >
                <Ear className={`mr-1 h-4 w-4 ${wakeListening ? 'animate-pulse' : ''}`} />
                {wakeListening ? '监听中' : '开启"糖豆"唤醒'}
              </Button>
              {/* 自动朗读开关 */}
              <Button
                variant="ghost"
