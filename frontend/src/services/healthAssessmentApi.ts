...
export async function getVisualizationData(
  ...
  const result = await apiRequest<VisualizationData>(`/api/health/visualization?${params}`);
  
  // API不可用时返回模拟数据
  if (!result.success) {
    console.log('使用模拟可视化数据');
    return { success: true, data: getMockVisualizationData() };
  }
  return result;
}
...
function getMockVisualizationData(): VisualizationData {
  return {
    radar_data: [
    ...
    trend_data: [
      ...
      { metric: '运动', direction: 'worsening', deviation: -8 },
    ],
    risk_distribution: {
      high: 1,
      medium: 2,
