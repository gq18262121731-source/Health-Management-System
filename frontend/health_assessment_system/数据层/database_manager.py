...
class DatabaseManager:
    ...
    def __init__(self, config: Dict = None):
        ...
        # 从环境变量读取配置（优先级最高）
        if os.getenv('DB_HOST'): self.db_config['host'] = os.getenv('DB_HOST')
        if os.getenv('DB_USER'): self.db_config['user'] = os.getenv('DB_USER')
        if os.getenv('DB_PASSWORD'): self.db_config['password'] = os.getenv('DB_PASSWORD')
