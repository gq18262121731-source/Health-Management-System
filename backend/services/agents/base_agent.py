...
class BaseAgent(ABC):
    ...
    def _execute_tools_if_needed(self, user_input: str, session_id: str = None) -> str:
        ...
        try:
            ...
            # 判断是否需要查询健康数据
            tool_triggers = {
                "query_health_records": ["最近血压", "血压记录", "血糖记录", "健康数据", "最近的数据", "查一下", "血压怎么样", "血糖怎么样", "我的血压", "我的血糖", "血压情况", "血糖情况", "心率怎么样", "我的心率"],
                "query_health_trend": ["血压趋势", "变化趋势", "这段时间", "最近怎么样", "这几天", "这周"],
                "query_recent_alerts": ["预警", "警报", "异常", "有什么问题", "有没有问题"],
                "query_medications": ["吃什么药", "用药", "药物", "提醒吃药", "吃药"],
            }
